...
def send_email(entries):
    if entries:
        ...
        s = smtplib.SMTP(host=os.environ["EMAIL_HOST"], port=os.environ["EMAIL_PORT"])

        s.starttls()
        s.login(user=os.environ["EMAIL_USERNAME"], password=os.environ["EMAIL_PASSWORD"])
...
def make_table(entries):
    ...
    for e in entries:
        ...
        if entries.index(e) == 0:
            output += (
                "".join("<th>{}</th>".format(html.escape(k.capitalize())) for k in e.keys())
                + "</tr><tr>"
            )
        for k, v in e.items():
            v = html.escape(v)
...
def parse_feeds(feeds):
    ...
    return sorted(
        [process_entry(e) for e in entries if within_interval(e)],
...
def process_entry(e):
    return {
        "timestamp": str(datetime.fromtimestamp(mktime(e.published_parsed))),
        "title": e.title,
        "link": e.link,
    }
